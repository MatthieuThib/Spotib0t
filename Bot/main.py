...
# Instancing discord bot and the spotify client
...
async def on_message(message):
    ...
    if(message.author.bot == False and not message.content.lower().startswith(('$'))):
        ...
        # Analyse message and send a response
            ...
            await message.add_reaction("🎵")
            await message.channel.send(" Working on it ... 🤔", delete_after = 5)
...
async def join(context):
    await context.send(" Hi there ! I'm **Spotibot** 😎. \nI'm your new best friend (yes I assure you 🐣).\nI love music on Spotify ! I know everything 🎧 \nWhat can I do for you ? 😁")
    
@bot.command(name = 'quit', aliases = ['q'])
async def quit(context):
    await context.channel.send(" Bye bye 👋")
...
@bot.command(name = "gif", aliases = ['g','Gif'])
